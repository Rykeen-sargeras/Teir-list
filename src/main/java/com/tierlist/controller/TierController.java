...
public class TierController {
    ...
    // PATCH lock/unlock image
    @PatchMapping("/images/{id}/lock")
    public ResponseEntity<Map<String, Object>> lockImage(
            @PathVariable Long id,
        ...
        if (body != null && body.get("locked") instanceof Boolean) {
            newLocked = (Boolean) body.get("locked");
        }
        img.setLocked(newLocked);
        repo.save(img);
        return ResponseEntity.ok(Map.of("id", img.getId(), "locked", img.isLocked()));
    }
